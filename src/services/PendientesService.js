...
exports.registrarPago = async (pagoId, pagoData) => {
  ...
  try {
    ...
    const nuevoMontoPagado =
      montoPagadoActual + parseFloat(pagoData.montoPagado);
    const nuevoMontoPendiente =
      montoPendienteActual - parseFloat(pagoData.montoPagado);
    ...
    if (pagoData.formaPago === "efectivo" || pagoData.formaPago === "mixto") {
      ...
      if (montoEfectivo > 0) {
        ...
        const montoInicial =
          lastCashStatus.rows.length > 0
            ? parseFloat(lastCashStatus.rows[0].monto_final)
            : 0;
